//import all_Functions.flattenJsonData
...
def flattenJsonData(path: String, parentNode: String = null): DataFrame = {
  ...
  val updatedDf: DataFrame = {
    if (null != parentNode && parentNode.trim.nonEmpty) {
      df.select(explode(col(parentNode)).as(parentNode))
        .select(s"$parentNode.*")
    } else
      df
  }
...
//  updatedDf.printSchema()
  ...
  iterateValues(updatedDf)
    //       .withColumn("EDH_UUID", lit(UUID.randomUUID().toString))
    .withColumn("EDH_UUID", uuid())
    .withColumn("EDH_TIMESTAMP", lit(Instant.now.getEpochSecond.toString))
}
...
// Full flattenJsonData Methods
    ...
    val flatJsonDf: DataFrame = flattenJsonData("C:\\Users\\S441633\\OneDrive - Emirates Group\\ENIT\\Code\\IssueLinksParse\\INPUT.json", parentNode.trim())
    println("Check 2")
//    println(flatJsonDf)
